...
<form method="POST" action="{{ route('login') }}">
    ...
    <div class="form-check text-left mb-3">
        ...
        <a href="{{ route('forgot-password') }}" class="fw-600 font-xsss text-grey-700 mt-1 float-right">{{ __('Forgot your Password?') }}</a>
    </div>
    ...
    <div class="col-sm-12 p-0 text-left mt-3">
        ...
        <h6 class="text-grey-500 font-xsss fw-500 mt-3 mb-0 lh-32">
            {{ __("Don't have account") }} <a href="{{ route('register') }}" class="fw-700 ms-1">{{ __('Register') }}</a>
        </h6>
    </div>
